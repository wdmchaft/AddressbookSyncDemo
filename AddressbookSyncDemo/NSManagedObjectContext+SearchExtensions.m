...
- (NSSet *)fetchObjectsForEntityName:(NSString *)newEntityName withPredicate:(id)stringOrPredicate, ... {
	...
	if (stringOrPredicate) {
		...
		if ([stringOrPredicate isKindOfClass:[NSString class]]) {
			...
			NSAssert2([stringOrPredicate isKindOfClass:[NSPredicate class]], @"Second parameter passed to %s is of unexpected class %@", sel_getName(_cmd), object_getClassName(stringOrPredicate));
